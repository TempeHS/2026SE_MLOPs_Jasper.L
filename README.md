...
2. Refactoring the business problem into a machine learning problem
   - We need to create a ML algorithm that can recieve data in a patients CRM record, and provide an accurate prediction of the progression of diabetes over the next 12 months. Our algorithm needs to be a regression algorithm capable of using multiple features to predict a value and it's change over a large time period. (**dont need to talk about where the data comes from, just talk about the algorithm. Also talk about how the model will be presented/given to users.**)
   - We need to create a regression ML algorithm capable of using multiple features to predict aa value over a large time. It needs a UI to interact with the algorithm.

3. Defining success metrics
   - Our model needs to perform well with unseen data, and performance must be consistent between training and testing data. Data must be presented in a form readable and useable by the end user/s. Algorithm needs to be integrated with the data in the CRM (data we are being provided).
